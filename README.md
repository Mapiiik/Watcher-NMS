...
(`DATA_ROOT`, the `app-data` volume by default) where the documents kept
against the records live. A dump of the database alone is therefore not a
backup.

```bash
bin/cake backup /somewhere/outside   # a directory named after the moment
bin/cake restore /somewhere/outside/2026-09-11-020000
```
