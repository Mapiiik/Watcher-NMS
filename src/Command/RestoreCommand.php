...
{
    /**
     * The connection a deployment has.
     */
    ...
    public function execute(Arguments $args, ConsoleIo $io)
    {
        try {
            ...
            // Asked before anything is written, so that a run which is going to refuse does it
            // before the files have been put back.
            $this->refuseUnlessEmpty(self::CONNECTION, $force);

            $this->unpackData($directory, $io);

            $this->restoreDatabase(self::CONNECTION, $directory, $force, $io);
